...
int currentStoneIndex; // CHANGED: renamed from 'index'
int Score=0;
int alienLife=100;
int GameLvl= 1;
...
// CHANGED: Added const to char* parameter
...
void DrawStone(int StoneIndex)
{
	...
	case 0:
		...
		glTranslated(xStone[currentStoneIndex] , yStone[currentStoneIndex] ,0);
...
// CHANGED: Replaced strrev with manual string reversal
void writeIntoFile() {						//To write high score on to file
	...
	if(fp!= NULL){
		...
		// Manual string reversal instead of strrev
		int len = strlen(temp);
		for(int j = 0; j < len/2; j++) {
			char tempChar = temp[j];
			temp[j] = temp[len - j - 1];
			temp[len - j - 1] = tempChar;
		}
...
void StoneGenerate(){
	...
	for(int i=0; i<MAX_STONES ;i++){
		currentStoneIndex = i; // CHANGED: using renamed variable
...
int main(int argc, char** argv) {
	...
	return 0; // CHANGED: Added return statement
}