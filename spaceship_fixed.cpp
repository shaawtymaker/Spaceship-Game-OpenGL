...
int curIndex;            // renamed from 'index' to avoid collision with builtin 'index'
int Score = 0;
int alienLife = 100;
int GameLvl = 1;
...
// portable strrev (not standard in GCC)
void strrev_local(char *s) {
    if (!s) return;
...
void displayRasterText(float x ,float y ,float z , const char *stringToDisplay) {
    int length;
    glRasterPos3f(x, y, z);
    length = (int)strlen(stringToDisplay);
...
void DrawStone(int StoneIndex)
{
    ...
    {
    case 0:
        glTranslated(xStone[curIndex] , yStone[curIndex] ,0);
...
bool checkIfSpaceShipIsSafe() {
    for(int i = 0; i < MAX_STONES; i++) {
        // Use logical && (and) for stoneAlive and then collision logic
        if (stoneAlive[i] && (
            (xOne >= (xStone[i]/2 -70) && xOne <= (xStone[i]/2 + 70) && yOne >= (yStone[i]/2 -18 ) && yOne <= (yStone[i]/2 + 53)) ||
            (yOne <= (yStone[i]/2 - 20) && yOne >= (yStone[i]/2 - 90) && xOne >= (xStone[i]/2 - 40) && xOne <= (xStone[i]/2 + 40))
            ))
        {   
            stoneAlive[i] = 0;
            return false;
...
void SpaceshipCreate(){    
    ...
    // removed stray glEnd() that had no matching glBegin()
    glPopMatrix();
}
